	...
	function load_script( $scripts = array() ) {
		...
		if( !empty($scripts) ) {
			foreach ($scripts as $script) {
				$stringify_script .= "<script type='text/javascript' src='".$CI->static_domain.'/resources/js/'.$script.".js?v=14'></script>";
		    }
		}
...
if ( ! function_exists('load_css')) {
	...
	function load_css( $scripts = array() ) {
		...
		if( !empty($scripts) ) {
			foreach ($scripts as $script) {
		        $stringify_script .= "<link href='".$CI->static_domain.'/resources/css/'.$script.".css?v=37' rel='stylesheet'>";
		    }
		}
	    echo $stringify_script;
	}
