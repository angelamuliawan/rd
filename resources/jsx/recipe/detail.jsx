var Root = React.createClass({
	...
	componentDidMount: function(){
		...
		var recipeIntro = $('#recipeIntro').text();
		$('#recipeIntro').html($.emoticons.replace(recipeIntro));
	...
	// _fetchCommentRegularly: function(){
	...
	_loadData: function( ajaxGlobal ) {
		...
		if( !this.state.onProcessingRequest && this.state.needToUpdateState ) {
			...
			this.setState({
				...
				$.ajax({
		            url: apiUri + this.props.url + '/' + this.state.recipeId,
		            type: 'GET',
		            dataType: 'json',
		            global: ajaxGlobal,
		            data: {
            ...
            success: function(data) {
            	if( data.status == 'success' ) {
					var removedIndex = this.state.data.findIndex(function(item){
						return item.FoodCommentID == obj.id
					});
            ...
            success: function(data) {
            	if( data.status == 'success' ) {
					var editedIndex = this.state.data.findIndex(function(item){
						return item.FoodCommentID == id
					});

				    this.setState({
				    	data: update(this.state.data, { [editedIndex] : {Comment: {$set: text}} }),
				    	onProcessingRequest: false,
				    	formEditComment: update(this.state.formEditComment, { 
		...
		$.ajax({
            ...
            success: function(data) {
            	if( data.status == 'success' ) {
				    ...
				    var editedIndex = this.state.data.findIndex(function(item){
						return item['FoodCommentID'] == itemId
					});
	...
	_handleCommentReplyEdit: function(obj){
		...
		var type = obj.type;
		var commentIndex = this.state.data.findIndex(function(item){
			return item['FoodCommentID'] == obj.commentId
		});
	...
	_handleReplyCommentToggleLike: function(obj){
		...
		$.ajax({
            ...
            success: function(data) {
            	if( data.status == 'success' ) {
					var editedReplyCommentIndex = this.state.data[commentIndex].ReplyComments.findIndex(function(item){
						return item['CommentReplyID'] == replyId
					});
	...
	render: function(){
		...
		if( !this.state.userId ) {
			loginToCommentButton = <Anchor href={ serviceUri + 'users/login' } title={ $.translate('comment_this_recipe') } className="btn btn-orange ml10 mb10 ajax-modal" />
		}
