var Root = React.createClass({
	...
	_loadData: function( ajaxGlobal ) {
		...
		if( !this.state.onProcessingRequest && this.state.needToUpdateState ) {
			...
			this.setState({
				limitTimeline : this.state.limitTimeline + 5
			}, function(){
				$.ajax({
		            ...
		            global: ajaxGlobal,
		            data: {
				        limit : this.state.limitTimeline,
    ...
    _appendNewData: function( data ){
        ...
        if( JSON.stringify(data.results) == JSON.stringify(this.state.dataTimeline) ) {
        	this.setState({
            ...
            this.setState({
            	dataTimeline : update(this.state.dataTimeline, {$set: data.results}),
